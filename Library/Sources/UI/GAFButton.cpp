...
bool GAFButton::init()
{
    ...
    if (result)
    {
        ...
        m_view->start();
        m_hitObject = m_view->getObjectByName(HIT_OBJECT_NAME);
    }
...
{
    cocos2d::Vec2 localPoint = m_view->convertToNodeSpace(pt);

    if (m_hitObject)
        return m_hitObject->getBoundingBoxForCurrentFrame().containsPoint(localPoint);
    else
        return m_view->getInternalBoundingBoxForCurrentFrame().containsPoint(localPoint);
}
